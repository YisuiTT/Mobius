...
class DDIMSampler(object):
    ...
    @torch.no_grad()
    def my_onestep(self, cond, shape, latents=None, timestep=None, index=None,
                     unconditional_guidance_scale=1., unconditional_conditioning=None,
                     **kwargs):
        device = self.model.betas.device
        b, _, f, _, _ = shape

        # timestep = torch.tensor(timestep.copy()).to(device=device, dtype=torch.long)  # [16]
        timestep = torch.full((b,), timestep, device=device, dtype=torch.long)
        noise_pred = self.unet(latents, cond, timestep,
                               unconditional_guidance_scale=unconditional_guidance_scale,
                               unconditional_conditioning=unconditional_conditioning,
                               **kwargs)

        latents, pred_x0 = self.ddim_step(latents, noise_pred, index)
    ...
    @torch.no_grad()
    def ddim_step(self, x, e_t, index):
        b, _, f, *_, device = *x.shape, x.device
